...
async function uploadPhoto(file) {
  if (file.size > 2*1024*1024) {
    ...
    const maxW = 1200, ratio = Math.min(maxW/img.width, maxW/img.height, 1);
...
const Btn = ({ children, onClick, variant="primary", style={}, disabled=false }) => (
  <button onClick={onClick} disabled={disabled} style={{ background:variant==="primary"?C.forest:variant==="danger"?"#d95050":"#efefef",color:variant==="primary"?"white":variant==="danger"?"white":C.dark,border:"none",borderRadius:12,padding:"13px 0",width:"100%",fontSize:15,fontWeight:700,cursor:disabled?"not-allowed":"pointer",fontFamily:"inherit",marginTop:6,opacity:disabled?0.6:1,...style }}>{children}</button>
);
...
function PhotoPicker({ value, onChange }) {
  ...
  const handleFile = async (file) => {
    if (!file) return;
    setUploading(true);
    try { const r = await uploadPhoto(file); onChange(r.url); }
    ...
    <div style={{ marginBottom:14 }}>
      ...
      {value ? (
        ...
        <div onClick={()=>!uploading&&inputRef.current.click()} style={{ border:"2px dashed #d0e8c8",borderRadius:12,padding:"28px",textAlign:"center",cursor:uploading?"wait":"pointer",background:"#f8fcf5" }}>
          {uploading ? <div style={{ color:C.forest,fontSize:14,fontWeight:600 }}>Subiendo foto...</div>
            : <><div style={{ fontSize:32,marginBottom:8 }}>📷</div><div style={{ fontSize:15,fontWeight:600,color:C.forest }}>Tocar para agregar foto</div><div style={{ fontSize:12,color:"#aaa",marginTop:4 }}>Cámara o galería</div></>}
        </div>
      )}
...
function AlumnosTab({ alumnos, pagos, turnos, onRefresh, tablet }) {
  ...
  const registrarPago = async () => {
    ...
    await supabase.from("pagos").insert({ alumno_id:modalPago.id, ciclo_key:ciclo.key, monto, fecha:pagoForm.fecha, metodo:pagoForm.metodo, items:pagoForm.items, observaciones:pagoForm.observaciones });
  ...
  // En tablet: lista en grilla de 2 columnas
  const gridCols = tablet ? "1fr 1fr" : "1fr";
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:14 }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Buscar..." style={{ flex:1,border:"1.5px solid #e8e8e8",borderRadius:12,padding:"11px 16px",fontSize:15,outline:"none",fontFamily:"inherit",background:"white" }}/>
        <select value={filter} onChange={e=>setFilter(e.target.value)} style={{ border:"1.5px solid #e8e8e8",borderRadius:12,padding:"11px 14px",fontSize:14,outline:"none",fontFamily:"inherit",background:"white" }}>
          <option value="todos">Todas</option><option value="pagados">Pagaron</option><option value="pendientes">Deben</option>
        </select>
      </div>

      {/* Lista — 2 columnas en tablet */}
      <div style={{ display:"grid",gridTemplateColumns:gridCols,gap:10,marginBottom:100 }}>
        {filtered.map(a=>{
      ...
      <Modal open={!!modalPago} onClose={()=>setModalPago(null)} title={`Pago — ${modalPago?.nombre}`}>
        {modalPago&&(<div>
          ...
          <Sel label="Método" value={pagoForm.metodo} onChange={e=>setPagoForm(p=>({...p,metodo:e.target.value}))}><option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option></Sel>
...
function FinanzasTab({ alumnos, pagos, encargos, movimientos, onRefresh, tablet }) {
  ...
  const addMov = async () => { if(!form.descripcion.trim()||!form.monto)return; setSaving(true); await supabase.from("movimientos").insert({tipo:form.tipo,categoria:form.categoria,descripcion:form.descripcion,monto:Number(form.monto),fecha:form.fecha,metodo:form.metodo,ciclo_key:cicloKey,observaciones:form.observaciones}); await onRefresh(); setSaving(false); setForm({tipo:"ingreso",categoria:"cuota",descripcion:"",monto:"",fecha:"",metodo:"transferencia",observaciones:""}); setModalAdd(false); };
  const delMov = async (id) => { await supabase.from("movimientos").delete().eq("id",id); await onRefresh(); };

  return (
    <div>
      <div style={{ marginBottom:14 }}>
        <select value={cicloKey} onChange={e=>setCicloKey(e.target.value)} style={{ width:"100%",border:"1.5px solid #e8e8e8",borderRadius:12,padding:"12px 16px",fontSize:15,fontWeight:700,outline:"none",fontFamily:"inherit",background:"white",color:C.dark }}>
...
export default function EucaliptaApp() {
  ...
  // En tablet: nav lateral izquierda en vez de barra inferior
  const navW = tablet ? 200 : 0;
  ...
  return (
    <div style={{ background:C.cream,minHeight:"100vh",fontFamily:"'Montserrat','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ display:"flex",flex:1 }}>
        {/* Nav lateral — solo tablet */}
        {tablet && (
          <div style={{ width:navW,flexShrink:0,background:"white",borderRight:"1px solid #f0ece6",padding:"20px 0",display:"flex",flexDirection:"column",gap:4,position:"sticky",top:57,height:"calc(100vh - 57px)",overflowY:"auto" }}>
            {TABS.map(t=>{
